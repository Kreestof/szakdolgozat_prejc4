...
namespace Trainbenchmark
{
    internal class Program
    {
            ...
            foreach (var b in triples)
            {
                switch (((UriNode)b.Object).Uri.Fragment)
                {
                    ...
                    case "#Route":
                        ...
                        int routeEntry = -1;
                        int routeExit = -1;
                        List<int> routeRequires = new List<int>();
                        List<int> routeFollows = new List<int>();
                        foreach (var triple in g.GetTriplesWithSubject(b.Subject))
                        {
                    ...
                    case "#Region":
                        ...
                        List<int> regionSensors = new List<int>();
                        List<int> regionElements = new List<int>();
                        foreach (var triple in g.GetTriplesWithSubject(b.Subject))
                        {
                            switch (triple.Predicate.ToString().Split('#')[1])
                            {
                    ...
                    case "#Segment":
                        ...
                        int segmentLength = -1;
                        List<int> segmentSemaphores = new List<int>();
                        foreach (var triple in g.GetTriplesWithSubject(b.Subject))
                        {
                            switch (triple.Predicate.ToString().Split('#')[1])
                            {
        ...
        private static Signal getSignalFromString(string s)
        {
            switch (s)
            {
                case "SIGNAL_FAILURE":
        ...
        private static Position getPositionFromString(string s)
        {
            switch (s)
            {
